...
def Profil():
    ...
    def Celakoncovka(radky,sloupce,meto=0):
        ...
        celkempoli = (radky*sloupce)*(radky*sloupce-1)*(radky*sloupce-1)
        ...
        def Preindexuj(seznam,veleseznam):
            def VelkyIndexing(seznam,veleseznam):
                ...
                ozna = 2**(2+(radky+sloupce)//2)
                for i in range(len(seznam)):
                    if i%ozna == 0:
                        tim = ti-1
                        ti = time.time()
                        print("Tridime",i,":",ti-tim-1,"sek. Celková velikost:",len(seznam))
                    if seznam[i][0] != "X" and seznam[i][1] != "X":
                        poop = seznam[i].pop()
        ...
        for n in range(2*(radky+sloupce)+6):
    ...
    """while len(bylo)!=169:
        ...
        Celakoncovka(a,b)"""
    for a in range(15+1):
        for b in reversed(range(15+1)):
            if [a,b] not in bylo and a >= 3 and b >= 3:
                print(a,b)
                bylo.append([a,b])
                Celakoncovka(a,b)
    #SPOUŠTĚNÍ:
    """a,b = 0,0
    while a == 0:
                  k = input("Zadej počet řádků šachovnice (3-9) ")
                  try:
                      k = int(k)
                      if k not in range(3,51):
    ...
    while b == 0:
                  l = input("Zadej počet sloupců šachovnice (3-26) ")
                  try:
                      l = int(l)
                      if l not in range(3,27):
                          print("Nepovolený rozsah")
                      else:
                        b = l
                  except:
